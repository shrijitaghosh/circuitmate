...
import { createSession, getHealth, sessionWebSocketPath, createPcmMicrophone, playBase64Audio } from "./backend.js";
...
export default function CircuitMateDashboard() {
  ...
  const [providerStatus, setProviderStatus] = useState({ deepgram: false, huggingFace: false, rime: false });
  ...
  const setVoiceState = useCallback((next, sub) => {
    ...
    setVoiceStateRaw(next); setSubText(sub || STATE_COPY[next].sub);
    setTickMode(next === "listening" ? "listen" : next === "speaking" ? "speak" : "off");
  }, []);

  useEffect(() => {
    clearInterval(tickIntervalRef.current);
    if (tickMode === "off") { setActiveTicks(new Array(TICKS).fill(null)); return; }
    const cls = tickMode === "listen" ? "listen" : "speak";
    tickIntervalRef.current = setInterval(() => setActiveTicks(new Array(TICKS).fill(null).map(() => Math.random() > 0.7 ? cls : null)), 150);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (disposed) return;
        setProviderStatus(health.providers || {}); setConnection(health.ok ? "Backend connected" : "Backend unavailable");
        ...
        socket.onerror = () => { setConnection("Backend error"); setVoiceState("idle", "Backend connection error"); };
        socket.onclose = () => setConnection("Disconnected");
        socket.onmessage = event => {
          const message = JSON.parse(event.data);
          if (message.type === "transcript.partial") { setUserText(message.text || ""); setVoiceState("listening"); }
          if (message.type === "transcript.final") { setUserText(message.text || ""); setVoiceState("processing"); }
          if (message.type === "facts.updated") {
            setCurrentGpio(message.facts?.pin || currentGpio); const item = message.corrections?.[0];
            if (item) { setCorrection(item); setProblemBadgeText("Correction applied"); setCorrectionFlash(false); requestAnimationFrame(() => setCorrectionFlash(true)); setIconSpin(true); setTimeout(() => setIconSpin(false), 900); }
          }
          if (message.type === "assistant.delta") { setAssistantPending(false); setAssistantText(message.text || ""); setAssistantTs(new Date().toTimeString().slice(0, 8)); setLatency(`${Date.now() - startedAtRef.current} ms`); setVoiceState("speaking"); }
          if (message.type === "tts.audio") audioRef.current = playBase64Audio(message.audioBase64, message.contentType);
          if (message.type === "turn.completed") { setLatency(`${message.latencyMs} ms`); setTimeout(() => { if (!disposed) { setVoiceState("idle", "Ready"); setProblemBadgeText("Diagnosing"); } }, 1200); }
          if (message.type === "turn.cancelled") { setVoiceState("idle", "Cancelled"); setAssistantPending(false); }
        };
      } catch { if (!disposed) { setConnection("Backend unavailable"); setVoiceState("idle", "Start the backend first"); } }
    })();
    return () => { disposed = true; clearInterval(tickIntervalRef.current); void microphoneRef.current?.stop(); socketRef.current?.close(); audioRef.current?.pause(); };
  }, [setVoiceState]);
  ...
  const runVoiceCycle = useCallback(async () => {
    if (stateRef.current === "listening") {
      ...
      setVoiceState("processing"); return;
    }
    if (stateRef.current !== "idle") {
      if (socketRef.current?.readyState === WebSocket.OPEN) socketRef.current.send(JSON.stringify({ type: "interrupt", reason: "user" }));
      audioRef.current?.pause(); setVoiceState("idle", "Ready"); return;
    }
    if (!socketRef.current || socketRef.current.readyState !== WebSocket.OPEN) { setVoiceState("idle", "Backend not ready"); return; }
    try {
      setUserText(""); setUserCorrectionSuffix(""); setUserTs(new Date().toTimeString().slice(0, 8)); setAssistantPending(true); setAssistantTs("—"); setCorrection(null); setProblemBadgeText("Diagnosing"); startedAtRef.current = Date.now();
      socketRef.current.send(JSON.stringify({ type: "turn.start", turnId: crypto.randomUUID() }));
      setVoiceState("listening");
      microphoneRef.current = await createPcmMicrophone(chunk => { if (socketRef.current?.readyState === WebSocket.OPEN) socketRef.current.send(chunk); });
    } catch { setVoiceState("idle", "Microphone permission required"); }
  }, [setVoiceState]);
