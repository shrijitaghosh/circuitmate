...
import { completeDebugSession, createDebugSession } from "./backend.js";
...
export default function CircuitMateDashboard() {
  ...
  const [tickMode, setTickMode] = useState("off");
  ...
  const [userTs, setUserTs] = useState("—");
  const [assistantPending, setAssistantPending] = useState(true);
  const [assistantText, setAssistantText] = useState("Speak a hardware problem to begin.");
  ...
  const [providerStatus, setProviderStatus] = useState({ deepgram: false, huggingFace: false, rime: false, livekit: false, });
  const [correction, setCorrection] = useState(null);
  const [liveKitStatus, setLiveKitStatus] = useState("checking");
// checking | available | connected | unavailable
  ...
  const publishEvent = useCallback(async (payload) => {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/livekit/config`);

    if (!response.ok) {
      throw new Error(`LiveKit config failed: ${response.status}`);
    }
    ...
    setProviderStatus(prev => ({
      ...prev,
      livekit: data.configured === true,
    }));

    console.log("LiveKit config:", data);
  } catch (error) {
  ...
  useEffect(() => {
    const beginDebugSession = async () => {
      try {
        const session = await createDebugSession();
        debugSessionRef.current = session.sessionId;
        localStorage.setItem("circuitmate_debug_session", session.sessionId);
      } catch (error) {
        console.warn("Debug session could not be created:", error);
      }
    };
    void beginDebugSession();
  }, []);
  const handleLiveKitData = useCallback((payload) => {
    const decoded = new TextDecoder().decode(payload);
    const message = safeJson(decoded);
    if (!message || typeof message !== "object") return;

    const type = message.type || message.event;

    if (type === "transcript.partial" || type === "user.transcript.partial") {
      setUserText(message.text || "");
      setVoiceState("listening");
      return;
    }

    if (type === "transcript.final" || type === "user.transcript.final") {
      setUserText(message.text || "");
      setVoiceState("processing");
      return;
    }

    if (type === "assistant.delta" || type === "assistant.transcript" || type === "assistant.message") {
      setAssistantPending(false);
      setAssistantText(message.text || message.content || "");
      setAssistantTs(new Date().toTimeString().slice(0, 8));
      if (startedAtRef.current) {
        setLatency(`${Date.now() - startedAtRef.current} ms`);
      }
      setVoiceState("speaking");
      return;
    }

    if (type === "facts.updated" || type === "fact.updated" || type === "correction.detected" || type === "correction.applied") {
      const facts = message.facts || {};
      const item = message.correction || message.corrections?.[0];

      if (facts.pin || facts.gpio) {
        setCurrentGpio(facts.pin || facts.gpio);
      }

      if (item) {
        setCorrection(item);
        if (item.next || item.correctedValue) {
          setCurrentGpio(item.next || item.correctedValue);
        }
        setProblemBadgeText("Correction applied");
        setCorrectionFlash(false);
        requestAnimationFrame(() => setCorrectionFlash(true));
        setIconSpin(true);
        window.setTimeout(() => setIconSpin(false), 900);
      }
      return;
    }

    if (type === "turn.completed" || type === "response.completed") {
      if (message.latencyMs != null) setLatency(`${message.latencyMs} ms`);
      window.setTimeout(() => {
    ...
    if (type === "turn.cancelled" || type === "response.cancelled") {
    ...
    if (type === "error") {
      console.error("CircuitMate agent error:", message);
      setVoiceState("idle", message.message || "Agent error");
    }
  }, [setVoiceState]);
  ...
  useEffect(() => {
    ...
    tickIntervalRef.current = setInterval(
      () => setActiveTicks(new Array(TICKS).fill(null).map(() => Math.random() > 0.7 ? cls : null)),
      150
    );
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      try {
        const authToken = localStorage.getItem("circuitmate_token");
        if (!authToken) throw new Error("CircuitMate authentication token not found");

        // Health is only used for the status bar. A healthy REST API is not
        // treated as a successful LiveKit connection.
        try {
  ...
  if (mountedRef.current) {
    setProviderStatus(prev => ({
      ...
      ...(health.providers || {}),
        ...
        setConnection("Creating LiveKit session…");
        ...
        if (!tokenResponse.ok) {
          ...
          throw new Error(`LiveKit token request failed (${tokenResponse.status}): ${detail}`);
        }
        ...
        room.on(RoomEvent.Connected, async () => {
          ...
          setConnection("LiveKit connected");
          try {
            await room.startAudio();
          } catch {
            // Browsers may require the first startAudio() call from a user gesture.
          }
        });
        ...
        room.on(RoomEvent.Disconnected, (reason) => {
          ...
          setConnection("LiveKit disconnected");
          setVoiceState("idle", "LiveKit disconnected");
        });
        ...
        room.on(RoomEvent.Reconnected, () => {
          if (mountedRef.current) setConnection("LiveKit connected");
        });

        room.on(RoomEvent.DataReceived, (payload) => {
          if (mountedRef.current) handleLiveKitData(payload);
        });
        ...
        room.on(RoomEvent.TrackSubscribed, (track) => {
          ...
          void audioElement.play().catch(() => {
            // Playback can be unlocked by clicking the microphone button.
          });
        });
        ...
        if (mountedRef.current) {
          setConnection("LiveKit unavailable");
          setVoiceState("idle", error.message || "Start the backend and LiveKit agent");
        }
      }
    };
  ...
  }, [handleLiveKitData, setVoiceState]);

  const runVoiceCycle = useCallback(async () => {
    const room = roomRef.current;

    if (!room || room.state !== "connected") {
      setVoiceState("idle", "LiveKit not ready");
      return;
    }

    // This user gesture is the best place to unlock browser audio playback.
    try {
      await room.startAudio();
    } catch {}
    ...
    if (stateRef.current === "listening") {
      try {
        ...
        await publishEvent({ type: "turn.commit" });
        setVoiceState("processing");
      } catch (error) {
        console.error("Unable to stop microphone:", error);
        setVoiceState("idle", "Microphone error");
      }
      return;
    }

    if (stateRef.current !== "idle") {
      await publishEvent({ type: "interrupt", reason: "user" });
      await room.localParticipant.setMicrophoneEnabled(false);
      setVoiceState("idle", "Ready");
      return;
    }
    ...
    try {
      ...
      setUserTs(new Date().toTimeString().slice(0, 8));
      setAssistantPending(true);
      setAssistantText("Listening for your hardware problem…");
      setAssistantTs("—");
      setCorrection(null);
      setProblemBadgeText("Diagnosing");
      ...
      setVoiceState("idle", "Microphone permission required");
    }
  }, [publishEvent, setVoiceState]);

  const copy = STATE_COPY[voiceState];

  const finishTroubleshooting = useCallback(async () => {
    try {
      if (debugSessionRef.current) await completeDebugSession(debugSessionRef.current);
      navigate("/success");
    } catch (error) {
      console.error("Unable to finish troubleshooting:", error);
      setVoiceState("idle", "Unable to save session");
    }
  }, [navigate, setVoiceState]);
  ...
  return (
    <div className="cmd-root">
      <style>{`
        ...
        .cmd-finish-btn{ margin-top: 18px; padding: 9px 14px; border: 1px solid var(--border); border-radius: 8px; background: transparent; color: var(--accent); font-family: var(--font-mono); font-size: 12px; }
        .cmd-finish-btn:hover{ background: var(--accent-soft); border-color: var(--border-hi); }
...
>
  ...
  Home
          ...
          <div className="cmd-topbar-right">
            <span className="cmd-session-tag">{sessionRef.current ? `Session #${sessionRef.current.sessionId.slice(0, 8)}` : "Session connecting"}</span>
            <span className="cmd-status-pill"><span className="cmd-dot cmd-pulse"></span>{connection}</span>
          </div>
        </div>
      </header>
      ...
      <main className="cmd-dashboard">
            ...
            <div className={`cmd-correction-card${correctionFlash ? " cmd-flash" : ""}`}>
              <div className="cmd-correction-top">
                ...
                <div className="cmd-correction-heading">
                  <div className="cmd-correction-title">{correction ? "Correction detected" : "Correction monitor"}</div>
                  <div className="cmd-correction-sub">{correction ? "Self-correction identified mid-sentence" : "Speak a correction to update the active fact"}</div>
                </div>
                <div className="cmd-badge cmd-badge-green">{correction ? "Correction accepted" : "Waiting"}</div>
              </div>
              ...
              <div className="cmd-correction-transition">
                ...
                <span className="cmd-value-new">{correction?.next || currentGpio}</span>
              </div>

              <div className="cmd-correction-detail-grid">
                <div>
                  <div className="cmd-cd-label">Original value</div>
                  <div className="cmd-cd-value cmd-strike">{correction?.previous || "—"}</div>
                </div>
                <div>
                  <div className="cmd-cd-label">Corrected value</div>
                  <div className="cmd-cd-value cmd-accent">{correction?.next || currentGpio}</div>
                </div>
                <div>
                  <div className="cmd-cd-label">Confidence</div>
                  <div className="cmd-confidence-bar"><div className="cmd-confidence-fill" style={{ width: `${Math.round((correction?.confidence || 0) * 100)}%` }}></div></div>
                  <div className="cmd-confidence-num">{correction ? `${Math.round(correction.confidence * 100)}%` : "—"}</div>
                </div>
              </div>

              <p className="cmd-correction-explain">{correction ? "CircuitMate detected a self-correction in the user’s speech and updated the active GPIO value." : "Correction events from the backend will appear here."}</p>
            </div>
        ...
        <section className={`cmd-voice-panel cmd-state-${voiceState}`}>
          ...
          <button className="cmd-finish-btn" type="button" onClick={finishTroubleshooting}>
            Mark issue resolved
          </button>
        </section>
      </main>
      ...
      <footer className="cmd-statusbar">
        <div className="cmd-statusbar-inner">
          <div className="cmd-sb-item"><span className="cmd-sb-label">Voice Engine</span><span className={`cmd-sb-value ${providerStatus.rime ? "cmd-ok" : ""}`}>{providerStatus.rime ? "Rime Active ✓" : "Rime unavailable"}</span></div>
          <div className="cmd-sb-item"><span className="cmd-sb-label">Speech Recognition</span><span className={`cmd-sb-value ${providerStatus.deepgram ? "cmd-ok" : ""}`}>{providerStatus.deepgram ? "Connected ✓" : "Unavailable"}</span></div>
          <div className="cmd-sb-item"><span className="cmd-sb-label">Backend</span><span className="cmd-sb-value">{connection}</span></div>
