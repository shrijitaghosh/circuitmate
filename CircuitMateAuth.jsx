...
async function mockAuthenticate(mode, fields) {
  await wait(1400);
  // Demo rule: any password containing "fail" triggers the error state,
  // so the error UI is easy to preview without wiring a real backend.
  if ((fields.password || "").toLowerCase().includes("fail")) {
    return { ok: false };
  }
  return { ok: true };
}
...
export default function CircuitMateAuth() {
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      const result = await mockAuthenticate(tab, fields);
      if (result.ok) {
        setAuthState("success");
        clearTimeout(successTimeoutRef.current);
        successTimeoutRef.current = setTimeout(() => {
          // Placeholder for real navigation to the dashboard.
          console.log("Navigate to CircuitMate dashboard");
        }, 1600);
      } else {
        setAuthState("error");
      }
    },
    [authState, tab, fields]
  );
            ...
            {authState === "error" && (
              <div className="cma-error-card">
                ...
                <div>
                  ...
                  <div className="cma-error-sub">Please check your email and password and try again.</div>
                </div>
              </div>
            )}
